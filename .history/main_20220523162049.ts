...
export default class TaskAggregator extends Plugin {
		...
		this.addCommand({
			...
			callback: () => {
				if (this.settingsAvailable()) {
					...
					new Notice('Settings not compatible.');
				}
			}
		});
	...
	async settingsAvailable(): boolean {
		const { vault } = this.app;

		const files: TFile[] = vault.getMarkdownFiles();

		const todoFileString: Promise<string> = vault.read(files.find(f => f.path == this.settings.todoFilePath));

		const now = new Date().toISOString().substring(0,10);
		const todayPath: string = this.settings.dailyFileFolder + now;
		const dailyFileString: Promise<string> = vault.read(files.find(f => f.path == todayPath));
