...
export default class MyPlugin extends Plugin {
	...
	async resetFiles(): Promise<void> {
		const {vault} = this.app;
		const files: string[] = await Promise.resolve(
			vault.getMarkdownFiles()
			// Array 4 TFile[]
			// basename: Todo List
		)

		const sourceFileNames: string[] = ["Todo List_copy",
		"2022-05-20_copy"];

		const sourceFiles: TFile[] = sourceFileNames.map(name => files.find(f => name));

		const targetFiles: string[] = ["Todo List",
		"2022-05-20"];

		sourceFiles.forEach((el, i) => {

			vault.copy()
			// copy el to targetFiles[i]
			// targetFiles[i]
		});
